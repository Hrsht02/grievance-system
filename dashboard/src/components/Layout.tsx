...
export default function Layout({ children, nav }: { children: ReactNode; nav: NavItem[] }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      <aside style={{ width: 220, background: "#1e3a5f", color: "white", display: "flex", flexDirection: "column", padding: "24px 0", flexShrink: 0 }}>
        ...
        <nav style={{ flex: 1, padding: "16px 0" }}>
          {nav.map((item) => (
            <Link key={item.path} to={item.path} style={{
              ...
              color: "white",
              borderLeft: location.pathname.startsWith(item.path) ? "3px solid #60a5fa" : "3px solid transparent",
            }}>
              {item.label}
            </Link>
          ))}
        </nav>
        <button onClick={logout} style={{ margin: "0 16px 16px", padding: "8px", background: "rgba(255,255,255,0.1)", border: "none", color: "white", borderRadius: 6, cursor: "pointer" }}>
          Sign Out
        </button>
      </aside>
